...
const DoctorsList = () => {
    ...
    const removeHandlerOne = (id: string) => {
        return (e: React.MouseEvent<SVGSVGElement>) => {
            e.preventDefault()
            setIdToRemove(id)
            twoStepRemoveScreen.current.style.display = 'block'
        }
    }
    ...
    const removeHandlerTwo = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault()
        remove(idToRevmoe)
        twoStepRemoveScreen.current.style.display = 'none'
    }
    ...
    return (
        <div className='container'>
            <div ref={twoStepRemoveScreen} style={{ display: 'none' }} className='w-50 border border-3 roudned rounded-3 mx-auto'>
            ...
            <div className='row justify-content-center'>
                <div className='col-8 col-md-5'>
                    <Link className='btn btn-outline-dark w-100 my-3' to={'/doctor/add'} state={{ from: loc }}>Add Doctor</Link>
                </div>
            </div>
            <h3>Doctors!</h3>
            ...
            <div className="input-group flex-nowrap my-3">
                ...
                <input type="text" className="form-control" placeholder="Username Prefix" aria-label="Username" aria-describedby="addon-wrapping"
                    ref={name} />
